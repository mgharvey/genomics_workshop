...
def get_samples(args):
	...
	print d.ix[d['lineage'] == args.lineage, 'sample'].tolist()
...
def get_sequences(args, samples):
	...
	for sample in samples:
		...
		seq = os.path.join(adir, '%s.contigs.fasta' % sample)
		s = open(seq, 'r')
		id = ''
...
def rev_comp(seq):
	seq = seq.upper()
	seq_dict = {'A':'T','T':'A','G':'C','C':'G','N':'N'} # MGH change
			...
			if d[5] in keep:
				...
				if c in match:
					...
					if float(d[6]) / eval < 1e3:
						...
						if len(seq[sample][d[0]].replace("N","")) > match[c]['len']: # MGH change
