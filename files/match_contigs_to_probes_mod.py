...
def get_query(args):
	...
	if args.query:
		...
		query = os.path.join(args.dir, '3_velvet-output/contigs', 
	                             '%s.contigs.fasta' % args.sample)
...
def sub_parse_blat(args, out, regex):
	matches = {}
        ...
        for l in f:
                ...
                #d[regex] = re.search('^([^_]+)', d[regex]).group(1) # Commented out to retain full db locus name
